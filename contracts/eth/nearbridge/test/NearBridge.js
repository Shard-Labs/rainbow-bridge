...
} = require('rainbow-bridge-lib/rainbow/borsh');
...
it('should be ok', async function () {
    ...
    await NearBridge.initWithValidators(
        borshifyInitialValidators(require('./block_120998.json').next_bps),
    );
    ...
    await expect(NearBridge.addLightClientBlock(block121998)).to.be.revertedWith(
            ...
            if (i >= 600) {
                ...
                for (let j = 0; j < block.approvals_after_next.length; j++) {
                    ...
                    if (block.approvals_after_next[j]) {
                        ...
                        expect(await NearBridge.checkBlockProducerSignatureInHead(j)).to.be
                            .true;
                    }
                }
            }
